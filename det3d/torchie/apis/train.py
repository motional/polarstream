...
def example_to_device_one_sector_torch(example, device=None, non_blocking=False) -> dict:
...
def example_to_device_one_sector_numpy(example, device=None, non_blocking=False) -> dict:
    assert device is not None
...
def train_detector(model, dataset, cfg, distributed=False, validate=False, logger=None):
    ...
    curriculum_weights = cfg.get('curriculum_weights', None)
    trainer = Trainer(
        model, batch_processor, optimizer, lr_scheduler, cfg.work_dir, cfg.log_level, curriculum_weights=curriculum_weights
    )

...
def example_to_device(example, device, non_blocking=False):
    if isinstance(example, dict):
        return example_to_device_one_sector_torch(example, device, non_blocking)
    elif isinstance(example, list):
        ret = []
        if len(example) >= 8: # more than 8 sectors
            example_to_device_one_sector = example_to_device_one_sector_numpy
        else:
            example_to_device_one_sector = example_to_device_one_sector_torch
        for ex in example:
            ret.append(example_to_device_one_sector(ex, device, non_blocking))
        return ret
    else:
        raise NotImplementedError
        
def example_to_device_one_sector_torch(example, device, non_blocking=False) -> dict:
    example_torch = {}
...
# an numpy version of loading data for 16 and 32 sectors. Otherwise there is memory error
def example_to_device_one_sector_numpy(example, device, non_blocking=False) -> dict:
    example_torch = {}
    float_names = ["voxels", "bev_map"]
    for k, v in example.items():
        if k in ["anchors", "anchors_mask", "reg_targets", "reg_weights", "labels", "hm",
                "anno_box", "ind", "mask", 'cat']:
            example_torch[k] = [torch.tensor(res).to(device, non_blocking=non_blocking) for res in v]
        elif k in [
            "voxels",
            "voxel_labels",
            "part_labels",
...
def parse_second_losses(losses, seg_loss_decay=1):

    log_vars = OrderedDict()
    loss = 0
    if 'det_loss' in losses:
        loss += sum(losses["det_loss"])
    if 'seg_loss' in losses:
        # dynamically balancing weights
        if seg_loss_decay < 0:
            seg_loss_decay = loss / (losses["seg_loss"][0].cpu().detach().numpy() + 1e-6)
        loss += sum(losses["seg_loss"]) * seg_loss_decay
    if 'part_loss' in losses:
        if seg_loss_decay < 0:
            seg_loss_decay = loss / (losses["part_loss"][0].cpu().detach().numpy() + 1e-6)
        loss += sum(losses["part_loss"]) * seg_loss_decay
...
class Trainer(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.curriculum_weights = kwargs.get('curriculum_weights', None)
        # Create work_dir
    ...
    def batch_processor_inline(self, model, data, train_mode, **kwargs):
        ...
        if train_mode:
            ...
            seg_loss_decay = 1
            if (self.curriculum_weights is not None) and ('seg_loss' in losses) and (self.epoch < len(self.curriculum_weights)):
                seg_loss_decay = self.curriculum_weights[self.epoch]
            self.call_hook("after_forward")
            loss, log_vars = parse_second_losses(losses, seg_loss_decay)
            del losses
    ...
    def train(self, data_loader, epoch, **kwargs):
        ...
        for i, data_batch in enumerate(data_loader):
            ...
            # if isinstance(data_batch, list) and (len(data_batch) > 8):
            #     ngroups = len(data_batch) // 8
            #     for i in range(ngroups-1):
            #         outputs = self.batch_processor_inline(
            #             self.model, data_batch[0:8], train_mode=True, **kwargs
            #         )
    ...
    def val(self, data_loader, **kwargs):
        ...
        if self.rank == 0:
            prog_bar = torchie.ProgressBar(len(data_loader))
        ...
        det_dict, seg_dict, pan_dict = self.data_loader.dataset.evaluation(
            final_dets, final_segs, output_dir=self.work_dir
        )

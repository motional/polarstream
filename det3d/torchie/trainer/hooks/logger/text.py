...
class TextLoggerHook(LoggerHook):
    ...
    def _log_info(self, log_dict, trainer):
        ...
        if trainer.world_size > 1:
            bbox_head = trainer.model.module.bbox_head
            seg_head = trainer.model.module.seg_head
            part_head = trainer.model.module.part_head
        else:
            bbox_head = trainer.model.bbox_head
            seg_head = trainer.model.seg_head
            part_head = trainer.model.part_head
        if bbox_head:
            class_names = bbox_head.class_names
            ...
            for idx, task_class_names in enumerate(class_names):
                ...
                trainer.logger.info(log_str)
        if seg_head:
            log_str = "segmentation loss: {} \n".format(log_dict['seg_loss'][0])
            trainer.logger.info(log_str)
        if part_head:
            log_str = "part loss: {} \n".format(log_dict['part_loss'][0])
            trainer.logger.info(log_str)

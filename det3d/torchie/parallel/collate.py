...
def collate_kitti(batch_list):
    if 'sweeps' in batch_list[0]:
        batch = []
        nsweeps = batch_list[0]['nsweeps']
        nsectors = batch_list[0]['nsectors']
        for i in range(nsweeps):
            samples = []
            for j in range(nsectors):
                samples += [sample['sweeps'][i*nsectors+j] for sample in batch_list]
            batch.append(collate_kitti_one_sector(samples))
        return batch
    elif 'sectors' in batch_list[0]:
        batch = []
        nsectors = len(batch_list[0]['sectors'])
        if nsectors >= 8:
            collate_one_sector = collate_kitti_one_sector_numpy
        else:
            collate_one_sector = collate_kitti_one_sector
        for i in range(nsectors):
            batch.append(collate_one_sector(sample['sectors'][i] for sample in batch_list))
        return batch
    else:
        return collate_kitti_one_sector(batch_list)

def collate_kitti_one_sector(batch_list):
    ...
    for key, elems in example_merged.items():
        if key in ["points", "voxels", "num_points", "num_gt", "voxel_labels", "num_voxels",
                   "cyv_voxels", "cyv_num_points", "cyv_num_voxels", "part_labels"]:
            ...
            ret[key] = res
        elif key in ["metadata", "valid_grid_ind", 'key_points_index']:
            ret[key] = elems
        elif key == "calib":
            ret[key] = {}
            for elem in elems:
        ...
        elif key in ["coordinates", "cyv_coordinates", "grid_ind"]:
            coors = []
...
# numpy version of collate_fn for 16 and 32 sectors. Otherwise there is memory error
def collate_kitti_one_sector_numpy(batch_list):
    example_merged = collections.defaultdict(list)
    for example in batch_list:
        if type(example) is list:
            for subexample in example:

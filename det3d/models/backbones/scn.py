...
class SpMiddleResNetFHD(nn.Module):
    def __init__(
        ...
        self.extra_sp_shape = kwargs.get('extra_sp_shape', [1, 0, 0])
        if norm_cfg is None:
            norm_cfg = dict(type="BN1d", eps=1e-3, momentum=0.01)
        ...
        padding = 0
        if self.extra_sp_shape[0] == 0:
            padding = 1
        self.conv4 = spconv.SparseSequential(
            SparseConv3d(
                64, 128, 3, 2, padding=[padding, 1, 1], bias=False
    ...
    def forward(self, voxel_features, coors, batch_size, input_shape):
        ...
        # input: # [41, 1600, 1408]
        sparse_shape = np.array(input_shape[::-1]) + self.extra_sp_shape

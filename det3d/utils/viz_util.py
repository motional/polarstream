...
def draw_lidar(pc,
    ...
    if color is None: color = pc[:,2]
    if pc_label: color = pc[:,3]
    if color_by_intensity: color = pc[:,2]
    #color = np.ones(pc[:,2].shape)
...
def draw_gt_boxes3d(gt_boxes3d, fig, color=(1,1,1), line_width=1, text=None, text_scale=(1,1,1), color_list=None):
    ...
    for n in range(num):
        ...
        if color_list is not None:
            color = color_list[n]
        if text is not None: mlab.text3d(b[4,0], b[4,1], b[4,2], '%s'%text, scale=text_scale, color=color, figure=fig)
        for k in range(0,4):

...
def _fill_trainval_infos(nusc, train_scenes, val_scenes, test=False, nsweeps=10, filter_zero=True):
    ...
    for sample in tqdm(nusc.sample):
        ...
        if test:
            ...
            panoptic_labels_filename = None
        else:
            lidarseg_labels_filename = os.path.join(nusc.dataroot,
                                                    nusc.get('lidarseg', ref_sd_token)['filename'])
            panoptic_labels_filename = os.path.join(nusc.dataroot,
                                                    nusc.get('panoptic', ref_sd_token)['filename'])
        ...
        info = {
            ...
            "lidarseg_path": lidarseg_labels_filename,
            "panoptic_path": panoptic_labels_filename,

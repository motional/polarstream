...
def create_groundtruth_database(
    ...
    **kwargs,
    ...
    if dataset_class_name in ["WAYMO", "NUSC"]: 
        if db_path is None:
            db_path = root_path / f"gt_database_{nsweeps}sweeps_withvelo_seg"
        if dbinfo_path is None:
            dbinfo_path = root_path / f"dbinfos_train_{nsweeps}sweeps_withvelo_seg.pkl"
    else:
        raise NotImplementedError()

    if dataset_class_name == "NUSC":
        point_features = 6 if 'seg' in pipeline[0]['super_tasks'] else 5
